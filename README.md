...
アルゴリズムによる判断をめぐる論点を、政治哲学の正義論に接続して整理した資料である。

「このアルゴリズムは公正か」という問いは、技術的な問いに見える。実際には「そもそも
公正とは何か」という古い問いを含む。統計的な指標をどれだけ精緻にしても、どの指標を
選ぶべきかは指標そのものからは決まらない。その選択の根拠は、正義をめぐる立場の違いに
遡る。

この資料は、**どの立場が正しいかを決めない**。論点ごとに、主要な立場から
どう見えるかを並べ、どこで意見が分かれるのかを示す。

## 読み方

はじめての方は **[この資料の読み方](docs/00-how-to-read.md)** から。
全体を見渡すなら **[対応表](docs/map.md)** が早いである。

### 正義をめぐる立場

何を最終的な基準に置くか。6 つの立場を扱う。
...
- 筆者は哲学と計算機科学の**学習者**であり、専門的な研究者ではない
- 各立場の要約は入門的な水準で、原典の議論を単純化している
- 文献表は主要なものに限っている。**論点編（`docs/issues/`）の 14 件は、DOI・arXiv 番号・
  一次 URL をすべて出版社側の記載と照合した**（照合方法は下記）。立場編（`docs/traditions/`）は
  古典的な単行本が中心のため、書誌情報のみで識別子は付していない

誤りの指摘や、立場の要約への異議は歓迎する。[CONTRIBUTING.md](CONTRIBUTING.md) を見ること。

### 識別子をどう確かめたか

記憶から書いていない。 一件ずつ検索し、**出版社・学会・索引の側が出している記載**と
突き合わせたものだけを載せている。判断基準は「DOI がその出版社の URL に埋まっているか」である
...
裏づけの取れた SSRN の DOI を採りた。 確認できないものを、それらしい形で書くよりは、
確認できたものを書くほうが正確である。

## 引用

この資料は Zenodo にアーカイブされ、DOI が付与されている。
...
`docs/` の Markdown から、読み物としての HTML を生成できる。

```bash
node build.js     # site/ に出力されます
```

依存パッケージはない。`main` への push で GitHub Actions が自動的に
ビルドし、GitHub Pages へ配信する。

生成したサイトは third-party へリクエストを出しない。 上のバッジのような外部画像は、
`build.js` が代替テキストに落とす —— GitHub 上ではバッジのまま、サイト上では
「Built with Claude Code」というリンクつきの文字列になる。AI 開示のような中身のある
記述が、画像の読み込み可否に左右されるべきではないためである。

```bash
node verification/check_site.js
```

生成物 17 ページを 5 項目で検査する。アスタリスクのまま残った強調、山括弧のまま残った
自動リンク、Markdown のまま残ったリンク記法、論点編に文献の識別子リンクがあること、そして
外部ホストから画像を読み込んでいないこと。push ごとに GitHub Actions が実行している。

この検査は後から足したものである。書いた直後に、バッジがサイト上で壊れて出ていたこと
（`![Built with Claude Code</a>](…)`）と、斜体がアスタリスクごと全ページに出ていたことが
分かった。 どちらも直してある。
...
使用して作成している。公開する文章の言い回しについて、**Grok**（xAI）に
候補を出させた。内容の確認、立場の要約の妥当性の判断、および
公開の可否は著者・根本卓哉（Takuya Nemoto）が行っている。AI は著作者ではない。

制作過程はリポジトリの履歴から確認できる（`git log --author=Claude`、
