...
アルゴリズムによる判断をめぐる論点を、政治哲学の正義論に接続して整理した資料です。

「このアルゴリズムは公正か」という問いは、技術的な問いに見えます。実際には「そもそも
公正とは何か」という古い問いを含みます。統計的な指標をどれだけ精緻にしても、どの指標を
選ぶべきかは指標そのものからは決まりません。その選択の根拠は、正義をめぐる立場の違いに
遡ります。
...
誤りの指摘や、立場の要約への異議は歓迎します。[CONTRIBUTING.md](CONTRIBUTING.md) を見ること。
...
HAL にも同じ本文を置いていましたが（`hal-05759717`）、2026-09-26 に削除しました。引用は Zenodo の DOI を用います。
...
生成したサイトは third-party へリクエストを出しません。 上のバッジのような外部画像は、
`build.js` が代替テキストに落とす —— GitHub 上ではバッジのまま、サイト上では
「Built with Claude Code」というリンクつきの文字列になります。AI 開示のような中身のある
記述が、画像の読み込み可否に左右されるべきではないためです。

```bash
node verification/check_site.js
```

生成物 17 ページを 5 項目で検査します。アスタリスクのまま残った強調、山括弧のまま残った
自動リンク、Markdown のまま残ったリンク記法、論点編に文献の識別子リンクがあること、そして
外部ホストから画像を読み込んでいないこと。push ごとに GitHub Actions が実行しています。

この検査は後から足したものです。書いた直後に、バッジがサイト上で壊れて出ていたこと
